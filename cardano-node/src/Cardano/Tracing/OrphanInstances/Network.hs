...
import           Network.TypedProtocol.Core (PeerHasAgency, PeerHasAgency' (..), SingPeerHasAgency' (..))
...
formatMessageWithAgency MaximalVerbosity msg _condensed =
  mconcat [ "kind" .= String (pack $ show msg)
          , "agency" .= String (pack $ show (sing :: Sing (PeerHasAgency st)))
          ]
...
instance Show (Sing st)
      => ToJSON (SingPeerHasAgency' ('PeerHasAgency st agency)) where
  toJSON (SingClientHasAgency stok) =
    Aeson.object [ "kind" .= String "ClientAgency"
                 , "agency" .= String (pack $ show stok)
                 ]
  toJSON (SingServerHasAgency stok) =
