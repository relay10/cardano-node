...
askNSetNodeState _window connectedNodes dpRequestors displayed = do
  ...
  forM_ connected $ \nodeId@(NodeId _anId) ->
    whenJustM (liftIO $ askDataPoint dpRequestors nodeId "NodeState") $ \(ns :: NodeState) -> do
      case ns of
        NodeTracingOnlineConfiguring -> return ()
        NodeOpeningDbs _ -> return ()
        NodeReplays _ -> return ()
        NodeInitChainSelection _ -> return ()
        NodeAddBlock (AddedToCurrentChain _ _ syncPct) -> setSyncProgress nodeId syncPct
        NodeStartup _ -> return ()
        NodeShutdown _ -> return ()
 where
