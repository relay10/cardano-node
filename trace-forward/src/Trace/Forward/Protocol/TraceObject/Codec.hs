...
import           Network.TypedProtocol.Codec (Codec, PeerHasAgency, SingPeerHasAgency' (..), SomeMessage (..))
 ...
 where
  ...
  decode = do
    ...
    case (key, len, stok) of
      (1, 3, SingClientHasAgency SingIdle) -> do
      ...
      (2, 1, SingClientHasAgency SingIdle) ->
        return $ SomeMessage MsgDone

      (3, 2, SingServerHasAgency (SingBusy blocking)) -> do
      ...
      (_, _, SingServerHasAgency (SingBusy SingBlocking)) ->
        fail (printf "codecTraceObjectForward (%s) unexpected key (%d, %d)" (show stok) key len)
      (_, _, SingServerHasAgency (SingBusy SingNonBlocking)) ->
        fail (printf "codecTraceObjectForward (%s) unexpected key (%d, %d)" (show stok) key len)
